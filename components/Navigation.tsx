  ...
  return (
    <nav
      ...
      className={cn(
        ...
        "rounded-full shadow-lg px-6 py-2 flex items-center justify-between gap-4",
        "w-auto",
      )}
    >
      {/* Menu toggle button */}
      <button
        ...
        className="p-2 rounded-full border border-white/20 hover:bg-white/20 dark:hover:bg-black/30 transition-colors sm:hidden text-gray-700 dark:text-gray-200"
      ...
      {isMenuOpen && (
        <div
          ...
          className="absolute top-16 left-1/2 -translate-x-1/2 w-[90%] bg-white dark:bg-black border border-white/20 rounded-2xl shadow-lg sm:hidden flex flex-col items-center py-4 gap-4"
        >
          {navItems.map((item) => {
            const sectionId = item.href.replace("#", "");
            return (
