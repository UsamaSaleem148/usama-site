...
export default function ContactForm() {
  ...
  const validate = (): boolean => {
    ...
    } else if (!/^[^\s@.]+(\.[^\s@.]+)*@[^\s@.]+(\.[^\s@.]+)*\.[a-zA-Z]{2,}$/.test(formData.email)) {
    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground max-w-sm mx-auto">
          Thank you for reaching out. I&apos;ll review your brief and respond
          within 24 hours.
        </p>
        <button
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <div>
        ...
        {errors.timeline && (
          <p className="mt-1 text-xs text-destructive">{errors.timeline}</p>
        )}
      </div>
      ...
      <p className="text-center text-xs text-muted-foreground">
        I respond to all inquiries within 24 hours. Your information is never
        shared.
      </p>
    </form>
  );
